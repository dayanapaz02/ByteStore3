...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $identidad = htmlspecialchars(trim($_POST['identidad']));
    $email = htmlspecialchars(trim($_POST['email']));
    $password = htmlspecialchars(trim($_POST['password']));
    $confirm_password = htmlspecialchars(trim($_POST['confirm_password']));

    // 2. Validaciones básicas
    if (empty($full_name) || empty($identidad) || empty($email) || empty($password) || empty($confirm_password)) {
    ...
    if ($stmt = $conn->prepare($check_email_sql)) {
        ...
        header("Location: ../registro.php?error=db_error_check_email");
        exit();
    }
    ...
    $insert_sql = "INSERT INTO users (full_name, identidad, email, password_hash) VALUES (?, ?, ?, ?)";
    ...
    if ($stmt = $conn->prepare($insert_sql)) {
        ...
        $stmt->bind_param("ssss", $full_name, $identidad, $email, $password_hash); // "ssss" porque son 4 strings
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            $conn->close();
            header("Location: ../login.html?registration=success"); // Mantener login.html por ahora, asumiendo que no se ha cambiado
            exit();
        } else {
            // Error al ejecutar la sentencia
            error_log("Error al ejecutar INSERT: " . $stmt->error);
            header("Location: ../registro.php?error=db_execution_failed");
            exit();
        }
    } else {
        ...
        header("Location: ../registro.php?error=db_prepare_failed");
        exit();
    }
