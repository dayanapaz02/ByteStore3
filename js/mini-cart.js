document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateTotals() {
        ...
        const isv = 0; // No calcular ISV aquí
        const total = subtotal - totalDiscount; // Solo suma productos y descuenta si hay
    ...
    function renderCartItems() {
        ...
        cart.forEach(item => {
            ...
            const discountCouponDisplay = item.couponDisplay ? `<div class="discount-coupon-tag">${item.couponDisplay}</div>` : ''; // Usa el texto del cupón si existe

            // DEBUG: Log la ruta de la imagen para verificar
            console.log('Intentando cargar imagen:', item.image);
    ...
    addProductButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const productData = {
                ...
                couponDisplay: productCard.dataset.couponDisplay || '', // Nuevo: Capturar el texto del cupón
    ...
    checkoutBtn.addEventListener('click', function() {
        // Calcula los totales justo antes de redirigir
        const totals = calculateTotals();
        // Guarda el total en localStorage para accederlo en la página de pago
        localStorage.setItem('byteStoreTotalToPay', totals.total.toFixed(2));
        localStorage.setItem('byteStoreCartISV', totals.isv.toFixed(2)); // También el ISV por si acaso
        localStorage.setItem('byteStoreCartSubtotal', totals.subtotal.toFixed(2)); // También el subtotal

        location.href = 'pago-cuotas.html'; 
    });
