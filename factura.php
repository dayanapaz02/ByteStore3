<?php
// Obtener datos de la factura desde la URL
$factura_numero = isset($_GET['numero']) ? $_GET['numero'] : date('Y') . str_pad(mt_rand(1, 9999), 4, '0', STR_PAD_LEFT);
...
if (!empty($productos)) {
    ...
    // Si no hay productos, usar el subtotal pasado por URL
...
<body>
        ...
        <div class="cliente-section">
            <div class="cliente-info">
                <div class="cliente-item">
                    <span class="cliente-label">Cliente:</span> <?php echo htmlspecialchars($cliente_nombre); ?>
                </div>
                <div class="cliente-item">
                    <span class="cliente-label">Dirección:</span> <?php echo htmlspecialchars($cliente_direccion); ?>
                </div>
                <div class="cliente-item">
                    <span class="cliente-label">Teléfono:</span> <?php echo htmlspecialchars($cliente_telefono); ?>
                </div>
                <div class="cliente-item">
    ...
    <script>
        // Auto-redirect después de 10 segundos si no se imprime
        setTimeout(function() {
        ...
        // Función para limpiar el carrito después de generar la factura
        function limpiarCarrito() {
            localStorage.removeItem('byteStoreCart');
            localStorage.removeItem('byteStoreTotalToPay');
            console.log('Carrito limpiado después de generar factura');
        }

        // Limpiar carrito cuando se carga la factura
        window.addEventListener('load', function() {
            // Solo limpiar si hay productos en la factura
